      ...
      <div class="mt-10 sm:mx-auto sm:w-full sm:max-w-sm">
        ...
        <form class="space-y-6" @submit.prevent="submit">
          <div>
            ...
            <div class="mt-2">
              <input type="name" name="name" id="name" required="" v-model="data.name" @focus="resetValidation()" class="block w-full rounded-md bg-white px-3 py-1.5 text-base text-gray-900 outline-1 -outline-offset-1 outline-gray-300 placeholder:text-gray-400 focus:outline-2 focus:-outline-offset-2 focus:outline-indigo-600 sm:text-sm/6" />
            </div>
            <p class="text-sm mt-1 text-red-600">
              {{ errorMessages.includes('name') ? errorMessages : '' }}
            </p>
          </div>
          <div>
